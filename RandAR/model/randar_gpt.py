...
class RandARTransformer(nn.Module):
    ...
    def configure_optimizer(
        ...
        return optimizer
    def variable_schedule_generate(
        self,
        cond: torch.Tensor,
    ...
    def generate_confidence_last(
        ...
        def probe_and_select(num_to_select: int, step_progress: int) -> torch.Tensor:
            ...
            _, local_topk = torch.topk(entropy, num_to_select, dim=1, largest=True)
    ...
    def generate_with_confidence(
        ...
        """Like generate_confidence_first(), but additionally tracks and returns
        the per-cell entropy and decode-step index at the moment each raster
        position was committed, for confidence-vs-step visualization.

        See generate_confidence_first() for the full algorithm description
        (this is a duplicate of that method with entropy/step bookkeeping
        added; token_order is ignored, same as generate_confidence_first).

        Returns:
            result_indices: [bsz, block_size] sampled tokens in raster order.
            result_entropy: [bsz, block_size] entropy (nats) of the winning
                probe distribution for each cell, in raster order.
            result_step: [bsz, block_size] the parallel-decoding step index
                (0-based) at which each cell was committed, in raster order.
        """
        bs = cond.shape[0]
        ###Creating token order just as a test:
        # Step-1: Generate the token orders and result sequences
        ...
        # [ADDED] entropy + step-index at commit time, raster order.
        result_entropy = torch.zeros((original_bs, self.block_size), dtype=torch.float, device=cond.device)
        result_step    = torch.zeros((original_bs, self.block_size), dtype=torch.long,  device=cond.device)
        ...
        # all block_size positions.  Each image independently selects its     #
        # num_to_select lowest-entropy UNFILLED positions.                    #
        # Returns sel: [original_bs, num_to_select] raster indices, and       #
        # sel_entropy: [original_bs, num_to_select] the winning entropy       #
        # values, aligned with sel.                                          #
        #                                                                     #
        # BUG FIX: previously averaged entropy across images (mean_H) and    #
        # returned a single shared 1-D selection for all images.              #
        # ------------------------------------------------------------------ #
        def probe_and_select(num_to_select: int, step_progress: int):
            # [CHANGE] num_unfilled is deterministic: the parallel-decoding
            # schedule fills the same count for every image each step, and
            # step_progress is exactly that cumulative count. Computing it
            # this way (vs. `(~filled_mask).sum(dim=1)[0].item()` +
            ...
            # map local candidate indices back to raster positions
            sel = torch.gather(candidate_sel, 1, local_sel)  # [original_bs, num_to_select]
            # [ADDED] winning entropy values, aligned with sel
            sel_entropy = torch.gather(entropy, 1, local_sel)  # [original_bs, num_to_select]
            return sel, sel_entropy
        ...
        # ------------------------------------------------------------------ #
        ###Switch to token order to isolate fid problem
        cur_sel, cur_sel_entropy = probe_and_select(num_query_token_cur_step, query_token_idx_cur_step)
        #cur_sel = token_order[:, query_token_idx_cur_step: query_token_idx_cur_step + num_query_token_cur_step]
        # cur_sel: [original_bs, 1]
        filled_mask.scatter_(1, cur_sel, True)
        ...
        while (query_token_idx_cur_step <= self.block_size - num_query_token_cur_step
            ...
            # same column for every image.
            result_indices.scatter_(1, cur_sel, indices)
            # [ADDED] record entropy + step index at commit time, aligned with cur_sel.
            result_entropy.scatter_(1, cur_sel, cur_sel_entropy)
            result_step.scatter_(1, cur_sel, torch.full_like(cur_sel, cur_inference_step))
            ...
            if query_token_idx_next_step < self.block_size and num_query_token_next_step > 0:
                #To isolate the issue CHANGE BACK LATER
                next_sel, next_sel_entropy = probe_and_select(num_query_token_next_step, query_token_idx_next_step)
                #next_sel = token_order[:, query_token_idx_next_step: query_token_idx_next_step + num_query_token_next_step]
                # next_sel: [original_bs, m_next]
                filled_mask.scatter_(1, next_sel, True)
            else:
                next_sel = torch.empty(original_bs, 0, dtype=torch.long, device=cond.device)
                next_sel_entropy = torch.empty(original_bs, 0, dtype=torch.float, device=cond.device)
            ...
            num_query_token_cur_step = num_query_token_next_step
            cur_sel = next_sel
            cur_sel_entropy = next_sel_entropy
